...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-[var(--color-popover)] p-6 text-[var(--color-popover-foreground)] shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    variants: {
      side: {
...
>(({ side = 'right', className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[var(--color-ring)] disabled:pointer-events-none">
        <X className="size-4" />
